...
} = require('../utils/schoolAbbreviation');
...
// ---------------------------------------------------------------------------
// POST /auth/signup  { name, schoolName, abbreviation, phoneNumber, email, password }
...
//
// THE ABBREVIATION IS ASKED FOR, not merely derived. It used to be computed
// from the school name behind the admin's back and only became visible later,
// in Settings. It is now the prefix on every receipt number the school issues
// ("CNPS042"), which makes it something the school should have agreed to before
// the first receipt goes out rather than something it discovers on one. The
// form still SUGGESTS the derived value so nobody has to invent one — it is
// prefilled and editable, not blank.
// ---------------------------------------------------------------------------
router.post('/signup', async (req, res) => {
  try {
    ...
    // REQUIRED, with the derived value as a fallback rather than as the answer.
    //
    // The fallback is not laxness: a client that predates this field would
    // otherwise be unable to create a school at all, and the derivation is
    // exactly what that client used to get. What it is not allowed to do is
    // produce something invalid — computeSchoolAbbreviation returns '' for a
    // name with fewer than two usable characters, and an empty prefix would
    // make every receipt this school issues unnumberable. Validated either way,
    // so the school row can never be created in a state where its first payment
    // is destined to fail.
    const abbreviation = normalizeSchoolAbbreviation(
      req.body?.abbreviation || computeSchoolAbbreviation(schoolName),
    );
    const invalidAbbreviation = validateSchoolAbbreviation(abbreviation);
    if (invalidAbbreviation) {
      return res.status(400).json({ code: 'INVALID_ABBREVIATION', error: invalidAbbreviation });
    }
    ...
    if (resumeTarget) {
      ...
      if (school) {
        // Still signup: this account has never been verified, so no receipt has
        // ever been issued under the abandoned attempt's abbreviation and there
        // is nothing to preserve. Taking the resubmitted value is the same
        // "once, at signup" write, not the later re-derivation that Settings
