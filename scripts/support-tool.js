...
const { computeSchoolAbbreviation, validateSchoolAbbreviation } = require('../src/utils/schoolAbbreviation');
...
async function forceVerifySignup(email, reason) {
  ...
  // Must match what POST /auth/signup sets — abbreviation is NOT NULL with no
  // column default, so omitting it here made this command throw outright rather
  // than create the account.
  //
  // DERIVED, because a PendingSignup has no abbreviation column to read: this
  // path predates the signup form asking for one, and the rows it recovers were
  // captured before the field existed. Derivation is what those signups would
  // have got anyway.
  //
  // But it is CHECKED, not assumed. The abbreviation is now the prefix on every
  // receipt this school will issue, and computeSchoolAbbreviation returns '' for
  // a name with fewer than two usable characters. Creating the school anyway
  // would produce an account that looks fine until its first payment is refused
  // for a reason nobody at the school can act on. Better to stop here, where a
  // support engineer is watching, and say what to do.
  const abbreviation = computeSchoolAbbreviation(pending.schoolName);
  const invalidAbbreviation = validateSchoolAbbreviation(abbreviation);
  if (invalidAbbreviation) {
    throw new Error(
      `Cannot derive a usable abbreviation from the school name ${JSON.stringify(pending.schoolName)}. `
      + `${invalidAbbreviation} Create the school through the signup form, which asks for one.`,
    );
  }
  ...
  await prisma.school.create({
    data: {
      ...
      abbreviation,
      abbreviationAutoGenerated: true,
      adminUserId: user.id,
